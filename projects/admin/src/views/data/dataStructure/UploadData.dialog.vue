...
function downloadTemplate () {
  browser.downloadCsv(
    fieldDesc.value.map((v) => {
      ...
      else if (v[0].includes('level') || v.includes('（variable）'))
  ...
  >
    <div flex="~ col gap10">
      <div flex="~ col gap4" text-sm font-400>
        ...
        <div p2 rounded-2 bg-grey-2>
          注意：【数据资源结构设置样例.csv】 中以一维表结构存储数据资源结构，导入到系统后，自动转换为树型结构，可以点击“查看按钮”查看上传的资源结构。【数据资源结构设置样例.csv】中的字段说明如下：
        </div>
        <table
          b-hidden rounded-2 overflow-hidden
          style="box-shadow: 0 0 0 1px var(--grey-3)"
        >
          <tr
            v-for="(row, i) in fieldDesc"
            :key="i"
            :bg="i % 2 === 0 ? 'grey-2' : 'grey-1'"
          >
            <td
              v-for="(col, j) in row"
              :key="j"
              p2 whitespace-nowrap
              :w="j === 0 ? '1px' : 'auto'"
              b="0.5px grey-3"
              v-text="col"
            />
