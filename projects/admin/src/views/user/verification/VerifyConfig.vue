...
const { verification, getVerificationConfig } = useSysConfig()
 ...
 * 是否可以编辑
 */
const isEdit = computed(() => adminRole.value?.includes(PermissionType.CONFIG_UPSERT_VERIFICATION))

/** 上传文件信息 */
const info = computed<(Omit<InfoItemProps, 'isEdit'> & {
  key: keyof Exclude<IConfigDto[SysConfig.VERIFICATION], undefined>
})[]>(() => {
  const sizeLimit = formatFileSize(verification.value?.sizeLimit ?? VERIFICATION_UPLOAD_DFT_SIZE_LIMIT)
  return [
    {
      label: '上传文件尺寸限制',
      key: 'sizeLimit',
    ...
    },
    {
      label: '上传文件可接受文件后缀',
      key: 'acceptLimit',
      modelValue: verification.value?.acceptLimit?.join('、'),
      arr: verification.value?.acceptLimit,
      type: 'fileSuffix',
    },
    {
      label: '上传文件提示信息',
      key: 'hint',
      modelValue: verification.value?.hint,
    },
    {
      label: '上传文件数量限制',
      key: 'amountLimit',
      modelValue: verification.value?.amountLimit?.toString(),
      text: verification.value?.amountLimit,
      type: 'number',
    }
  ]
})
...
async function initPage() {
  ...
  try {
    await getVerificationConfig(false)
  }
  finally {
    loading.value = false
  }
...
async function reset(item: typeof info.value[number]) {
  ...
  try {
    await upsertConfigApi({
      version: SysConfig.VERIFICATION,
      verification: {
        ...verification.value,
        [item.key]: undefined
      }
...
async function update(item: typeof info.value[number], val: UpdateParam) {
  ...
  try {
    ...
    if (verification.value)
      verification.value[item.key] = (type === 'fileSuffix' ? arr?.filter(Boolean) : text) as any
  }
  finally {
    loading.value = false
  }
